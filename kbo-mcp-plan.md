...
| 기능 | 방식 | 엔드포인트 |
|---|---|---|
| 일정 | JSON API (form POST) | `https://www.koreabaseball.com/ws/Schedule.asmx/GetMonthSchedule` |
...
- [ ] `pyproject.toml` 작성
- [ ] `venv` 생성 및 의존성 설치 (`mcp[cli]`, `httpx`, `beautifulsoup4`, `lxml`, `pydantic`)
...
- [ ] 동작 확인: 순위 페이지 200 응답, 일정 API JSON 응답
...
- [ ] JSON `rows[].row[].Text` 안의 HTML 파싱
- [ ] `<li class="dayNum">` 에서 날짜 추출
- [ ] `"AWAY : HOME [구장]"` 패턴 정규식으로 경기 추출
- [ ] 팀 약어 → 전체 팀명 매핑 (`TEAM_MAP`)
- [ ] 경기취소/우천취소 `status` 처리
- [ ] 라이브 API 응답으로 파싱 결과 직접 확인
...
leId=1&srIdList=0&seasonId=2026&gameMonth=03&teamId=0
```

- `Content-Type: application/json` 이 아닌 **form-encoded** 필수
...
| `gameMonth` zero-pad 여부 | `"03"` 형식으로 확인됨 |
| KBO 사이트 HTML 구조 변경 | `<table class="tData">` selector 유지 모니터링 |
